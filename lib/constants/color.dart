import 'package:flutter/cupertino.dart';

const Color top_icon_1 = Color(0xFF11A9FF);
const Color top_icon_2 = Color(0xFF00EC89);

const Color mth = Color(0xFFAF31FC);

const Color phy = Color(0xFF31FCA7);

const Color db = Color(0xFFD36B69);

const Color green_light = Color(0x00EF99);

const Color primaryColor = Color(0xFF3185FC);
const Color secondaryColor = Color(0xFFF7CB15);

const Color correct = Color(0xFF12E667);
const Color incorrect = Color(0xFFED254E);
const Color missed = secondaryColor;

const Color resultCircle1 = Color(0xFF18FE9E);
const Color resultCircle2 = Color(0xFF11A9FF);
const Color timerColor = Color(0xFFF9E26A);

const Color grey = Color(0xFF838181);
